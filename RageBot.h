...
public:
	...
	void quickstop(CInput::CUserCmd * cmd, C_BaseEntity * local);
	void plugwalk(CInput::CUserCmd * cmd, C_BaseEntity * local, CBaseCombatWeapon* pWeapon);
	static void FakeWalk(CInput::CUserCmd * cmd, bool & bSendPacket, C_BaseEntity * local);
	static void FakeLag(bool & bSendPacket);
